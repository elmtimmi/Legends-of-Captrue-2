...
public class babaren : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        GetComponent<TextMesh>().text = "" + a.babaren[z * 100 + x];
    ...
    public void map()
    {
        if (this.transform.name == "Babaren")
        {
            if (a.map == 1)
            {
                for (int i = 1; i < 6; i++)
                {
                    for (int t = 1; t < 6; t++)
                    {
                        x = t;
                        z = 6 - i;
                        Instantiate(this, new Vector3(t * 100 - 80, 10, i * 100 - 50), this.transform.rotation);
                    }
                }
            }
            if (a.map == 2)
